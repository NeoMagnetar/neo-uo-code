...
namespace Server.Custom.AIGM
{
    public sealed class AIGMCompanionCognitionSnapshot
    {
        ...
        public string CompanionModeReason { get; set; }
        public string CompanionModeChangedAge { get; set; }
        public string UMGPersonaContext { get; set; }
        ...
        public AIGMCompanionCognitionSnapshot()
        {
            ...
            QuietMode = false;
            CompanionMode = "IDLE";
            CompanionModeReason = "unset";
            CompanionModeChangedAge = "unset";
            UMGPersonaContext = "none";
        }
        ...
        public static AIGMCompanionCognitionSnapshot Build(Mobile companion, AIGMCompanionSpeechRequest request)
        {
            ...
            ApplyCompanionMode(snapshot, companion);
            snapshot.IntentSummary = BuildIntentSummary(request);
            snapshot.CapabilitySummary = BuildCapabilitySummary(request);
            snapshot.UMGPersonaContext = request != null && !String.IsNullOrWhiteSpace(request.UMGPersonaContextSummary) ? request.UMGPersonaContextSummary : "none";
            return snapshot;
        }

        private static void ApplyCompanionMode(AIGMCompanionCognitionSnapshot snapshot, Mobile companion)
        {
            AIGMCompanionModeState state = AIGMCompanionModeService.GetSnapshot(companion);
            snapshot.CompanionMode = AIGMCompanionModeService.FormatMode(state.Mode);
            snapshot.CompanionModeReason = Safe(state.Reason);
            snapshot.CompanionModeChangedAge = state.ChangedUtc == DateTime.MinValue
                ? "unset"
                : Math.Max(0, (int)(DateTime.UtcNow - state.ChangedUtc).TotalSeconds) + "s ago";
        }
        ...
        private static string BuildPackSummary(Mobile companion)
        {
            ...
            return BuildPackStateSummary(companion, parts.Count > 0 ? String.Join(", ", parts.ToArray()) : "no relevant pack items noted");
        }

        private static string BuildPackStateSummary(Mobile companion, string itemSummary)
        {
            Container pack = companion != null ? companion.Backpack : null;
            string burden = "burden=unknown";
            if (pack != null && pack.MaxWeight > 0)
            {
                double ratio = (double)pack.TotalWeight / (double)pack.MaxWeight;
                if (ratio >= AIGMCompanionInventoryPolicy.HardThreshold)
                    burden = "burden=hard";
                else if (ratio >= AIGMCompanionInventoryPolicy.SoftThreshold)
                    burden = "burden=soft";
                else
                    burden = "burden=ok";
                burden += String.Format(" weight={0}/{1}", pack.TotalWeight, pack.MaxWeight);
            }

            string autoLoot = AIGMCompanionAutoLootService.IsEnabled(companion) ? "autoloot=on" : "autoloot=off";
            string potions = AIGMCompanionPotionService.IsPotionSupportEnabled(companion) ? "potions=on" : "potions=off";
            string spells = AIGMCompanionSpellService.IsSpellSupportEnabled(companion) ? "spells=on" : "spells=off";
            string potionCounts = AIGMCompanionPotionService.BuildPotionCountSummary(companion);
            return String.Format("{0}; {1}; {2}; {3}; {4}; potionCounts={5}", itemSummary, burden, autoLoot, potions, spells, potionCounts);
        }
