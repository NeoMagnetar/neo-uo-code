...
namespace Server.Custom.AIGM
{
    public static class AIGMCompanionCombatController
    {
        ...
        private static bool IssueNativeKillOrder(BaseHire companion, Mobile target, string targetKind, out string result)
        {
            ...
            string bridgeDetail;
            AIGMNativeCombatBridgeResult bridgeResult = AIGMNativeCombatBridge.TryStartNativeCombat(
                companion,
                target,
                commander,
                -1,
                "old_companion:" + targetKind,
                out bridgeDetail);

            if (bridgeResult != AIGMNativeCombatBridgeResult.Started && bridgeResult != AIGMNativeCombatBridgeResult.AlreadyEngaged)
            {
                result = bridgeDetail;
                return false;
            }
