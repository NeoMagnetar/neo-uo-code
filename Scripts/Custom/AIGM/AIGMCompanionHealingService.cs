...
namespace Server.Custom.AIGM
{
    public static class AIGMCompanionHealingService
    {
        ...
        private static bool StartCommandedHealing(BaseHire healer, Mobile target, out string response)
        {
            ...
            SetHealingMode(healer, target, "commanded_healing");
        ...
        private static bool TryBeginBandage(BaseHire healer, Mobile target, out string response)
        {
            ...
            SetHealingMode(healer, target, healer == target ? "self_bandage" : "bandage_target");
        ...
        private static bool StopCommandedHealing(BaseHire healer, string reason, bool explicitStop)
        {
            ...
            ClearHealingModes(healer, reason ?? "stop_healing");
            return wasActive;
        }

        private static void SetHealingMode(BaseHire healer, Mobile target, string reason)
        {
            if (!IsValidHealer(healer) || target == null)
                return;

            AIGMCompanionMode mode = AIGMCompanionMode.SupportBandageTarget;
            if (target == healer)
                mode = AIGMCompanionMode.SupportBandageSelf;
            else if (target == healer.GetOwner())
                mode = AIGMCompanionMode.SupportOwnerMedic;

            AIGMCompanionModeService.SetMode(healer, mode, reason);
        }

        private static void ClearHealingModes(BaseHire healer, string reason)
        {
            AIGMCompanionModeService.ClearIfAny(
                healer,
                reason,
                AIGMCompanionMode.SupportBandageSelf,
                AIGMCompanionMode.SupportBandageTarget,
                AIGMCompanionMode.SupportOwnerMedic);
        }
