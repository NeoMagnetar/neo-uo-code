...
namespace Server.Custom.AIGM
{
    public static class AIGMCompanionTrackingService
    {
        ...
        public static string StartTracking(BaseHire companion, Mobile speaker, AIGMCompanionTrackingMode mode)
        {
            ...
            AIGMCompanionModeService.SetMode(companion, AIGMCompanionMode.TrackMonster, "tracking_awareness_start");
            return BuildStartLine(companion, state);
        }

        public static string StopTracking(BaseHire companion, Mobile speaker)
        {
            return StopTracking(companion, speaker, true);
        }

        public static string StopTracking(BaseHire companion, Mobile speaker, bool restorePursuitAuthority)
        {
            if (!IsValidCompanion(companion))
            ...
            bool restored = restorePursuitAuthority && RestorePursuitAuthority(companion, state, "stop_tracking");
            ClearActionTarget(state);
            if (!restored && restorePursuitAuthority)
                AIGMCompanionCombatController.StopCombat(companion);
            else
            {
                companion.Combatant = null;
                companion.FocusMob = null;
                companion.Warmode = false;
                if (!restorePursuitAuthority)
                {
                    companion.ControlTarget = null;
                    companion.ControlOrder = OrderType.Stay;
                }
            }
            AIGMExecutionLog.Write("AIGM_TRACK_STOP companion={0} active={1} reason={2}", companion.Serial.Value, state.IsActive, SafeLog(state.StopReason));
            ...
            ClearTrackingModes(companion, "stop_tracking");
        ...
        public static string StartTrackingAction(BaseHire companion, Mobile speaker, AIGMCompanionTrackingMode mode, AIGMCompanionTrackingActionMode actionMode)
        {
            ...
            if (!AIGMOperationalControlService.BeginExplicitCommand(companion, speaker, "tracking:" + actionMode, out operationalRejection))
                return "Operational control rejected tracking: " + operationalRejection + ".";
            ...
            if (actionMode == AIGMCompanionTrackingActionMode.TrackOnly)
            {
                ...
                AIGMCompanionModeService.SetMode(companion, AIGMCompanionMode.AssessmentExplicit, "tracking_report");
            ...
            AIGMExecutionLog.Write("AIGM_TRACK_START companion={0} active={1} mode={2} action={3}", companion.Serial.Value, state.IsActive, state.Mode, state.ActionMode);
            if (state.ActionMode == AIGMCompanionTrackingActionMode.TrackHunt)
                AIGMExecutionLog.Write("AIGM_TRACK_TO_HUNT companion={0} mode={1} target={2}", companion.Serial.Value, state.Mode, state.CurrentTargetSerial);
            ...
            SetTrackingMode(companion, actionMode, "tracking_action_start");
            return tick;
        }
        ...
        public static string StartTrackingActionFromIntent(BaseHire companion, Mobile speaker, AIGMCompanionIntent intent)
        {
            ...
            if (kind == AIGMCompanionIntentKind.StopTrackingCycle || kind == AIGMCompanionIntentKind.StopMonsterHunt)
                return StopTracking(companion, speaker, false);
        ...
        public static string GetTrackingStatus(BaseHire companion, Mobile speaker)
        {
            ...
            if (!state.IsActive)
            {
                AIGMExecutionLog.Write("AIGM_TRACK_STATUS companion={0} active=False mode={1} action={2} target={3}", companion.Serial.Value, state.Mode, state.ActionMode, state.CurrentTargetSerial);
            ...
            AIGMExecutionLog.Write("AIGM_TRACK_STATUS companion={0} active=True mode={1} action={2} target={3} nearest=\"{4}\"", companion.Serial.Value, state.Mode, state.ActionMode, state.CurrentTargetSerial, SafeLog(BuildNearestTrailSummary(state)));
        ...
        public static string BuildTrackingSweepReport(BaseHire companion, Mobile speaker, AIGMCompanionTrackingMode mode)
        {
            ...
            state.LastScanUtc = DateTime.UtcNow;
            AIGMExecutionLog.Write("AIGM_TRACK_SCAN companion={0} mode={1} range={2} accepted=\"{3}\" rejected=\"{4}\"", companion.Serial.Value, mode, GetTrackingRange(companion), SafeLog(selection.AcceptedSummary), SafeLog(selection.RejectedSummary));
            ...
            if (selection.SelectedTarget == null)
            {
                ...
                AIGMExecutionLog.Write("AIGM_TRACK_NO_TARGET companion={0} mode={1}", companion.Serial.Value, mode);
            ...
            state.LastKnownDistanceText = selection.SelectedDistanceText;
            state.LastKnownTileText = selection.SelectedTileText;
            AIGMExecutionLog.Write("AIGM_TRACK_TARGET_FOUND companion={0} mode={1} target={2} name=\"{3}\" direction=\"{4}\" distance=\"{5}\" tile=\"{6}\"", companion.Serial.Value, mode, selection.SelectedTarget.Serial.Value, SafeLog(selection.SelectedName), SafeLog(selection.SelectedDirection), SafeLog(selection.SelectedDistanceText), SafeLog(selection.SelectedTileText));
        ...
        public static void Pulse(BaseHire companion)
        {
            ...
            if (!AIGMOperationalControlService.CanOperate(companion, AIGMOperationalAction.Tracking, AIGMOperationalControlService.GetEpoch(companion), false))
                return;
        ...
        public static string TickTrackingAction(BaseHire companion)
        {
            ...
            int expectedEpoch = AIGMOperationalControlService.GetEpoch(companion);
            if (!AIGMOperationalControlService.CanOperate(companion, AIGMOperationalAction.Tracking, expectedEpoch, false))
                return String.Empty;

            AIGMExecutionLog.Write("AIGM_TRACK_ACTIVE companion={0} mode={1} action={2} target={3}", companion.Serial.Value, state.Mode, state.ActionMode, state.CurrentTargetSerial);
            ...
            if (state.ActionMode == AIGMCompanionTrackingActionMode.TrackHunt)
            {
                ...
                {
                    LogTrackingMovePrimitive(companion, target, "CombatantChase", "TryEngageTarget", "TrackingService");
                ...
                if (companion.ControlTarget == target && companion.ControlOrder == OrderType.Attack)
                {
                    ...
                    LogTrackingMovePrimitive(companion, target, "CombatantChase", "CombatSustain", "TrackingService");
                    LogTrackingMoveTick(companion, target, companion.Location, currentDistance, state, "CombatantChase");
        ...
        private static string StartBlockedHunt(BaseHire companion, Mobile speaker, AIGMCompanionTrackingMode mode)
        {
            ...
            AIGMCompanionModeService.SetMode(companion, AIGMCompanionMode.RefusalPlayerHunt, "tracking_hunt_blocked");
            return "I can find them, but I will not take steel to them without a stronger order and proper authority.";
        }

        private static void SetTrackingMode(BaseHire companion, AIGMCompanionTrackingActionMode actionMode, string reason)
        {
            if (!IsValidCompanion(companion))
                return;

            AIGMCompanionModeService.SetMode(
                companion,
                actionMode == AIGMCompanionTrackingActionMode.TrackHunt ? AIGMCompanionMode.HuntMonster : AIGMCompanionMode.TrackMonster,
                reason);
        }

        private static void ClearTrackingModes(BaseHire companion, string reason)
        {
            AIGMCompanionModeService.ClearIfAny(
                companion,
                reason,
                AIGMCompanionMode.TrackMonster,
                AIGMCompanionMode.HuntMonster,
                AIGMCompanionMode.RefusalPlayerHunt);
        }
        ...
        private static bool TryStepTowardTarget(BaseHire companion, Mobile target, AIGMCompanionTrackingState state)
        {
            ...
            {
                state.PursuitPathFollower = CreatePathFollower(companion, target);
            ...
            LogTrackingMovePrimitive(companion, target, "NativeAI", "PathFollower.Follow", "TrackingService");
            if (TryPathFollowerStep(companion, target, state, before, beforeDistance))
            {
                LogTrackingMoveTick(companion, target, before, beforeDistance, state, "PathFollower.Follow");
                return true;
            }
            ...
            for (int i = 0; i < candidates.Length; i++)
            {
                ...
                LogTrackingMovePrimitive(companion, target, "CustomStep", "TryMove", "TrackingService");
                if (TryMove(companion, direction))
                {
                    RecordPursuitStep(companion, target, state, before, beforeDistance, "direct_step");
                    LogTrackingMoveTick(companion, target, before, beforeDistance, state, "TryMove");
                    return true;
                }
                ...
                if (door.Opened && TryMove(companion, direction))
                {
                    ...
                    LogTrackingMoveTick(companion, target, before, beforeDistance, state, "TryMoveAfterDoor");
                    return true;
                }
            }
            ...
            LogTrackingMoveTick(companion, target, before, beforeDistance, state, "blocked");
            return false;
        }
        ...
        private static bool TryMove(BaseHire companion, Direction direction)
        {
            ...
            {
                MoveResult result = companion.AIObject.DoMoveImpl(move);
        ...
        private static PathFollower CreatePathFollower(BaseHire companion, IPoint3D target)
        {
            PathFollower follower = new PathFollower(companion, target);

            if (companion != null && companion.AIObject != null)
                follower.Mover = companion.AIObject.DoMoveImpl;

            return follower;
        }

        private static void LogTrackingMovePrimitive(BaseHire companion, Mobile target, string primitive, string methodName, string owner)
        {
            AIGMExecutionLog.Write(
                "AIGM_TRACK_MOVE_PRIMITIVE companion={0} targetMobile={1} primitive={2} methodName={3} owner={4}",
                DescribeMobile(companion),
                DescribeMobile(target),
                SafeLog(primitive),
                SafeLog(methodName),
                SafeLog(owner));
        }

        private static void LogTrackingMoveTick(BaseHire companion, Mobile target, Point3D previousPoint, int previousDistanceToTarget, AIGMCompanionTrackingState state, string movementPrimitive)
        {
            if (companion == null || target == null || state == null)
                return;

            int currentDistance = state.LastMoveDistanceAfter >= 0
                ? state.LastMoveDistanceAfter
                : (int)Math.Round(companion.GetDistanceToSqrt(target));
            int distanceDelta = previousDistanceToTarget - currentDistance;
            BaseCreature creature = companion as BaseCreature;
            AIGMExecutionLog.Write(
                "AIGM_TRACK_MOVE_TICK companion={0} targetMobile={1} companionPoint={2} targetPoint={3} distanceToTarget={4} previousDistanceToTarget={5} distanceDelta={6} movementPrimitive={7} controlOrder={8} controlTarget={9} combatant={10} aiAction={11} direction={12} movedThisTick={13}",
                DescribeMobile(companion),
                DescribeMobile(target),
                FormatPoint(companion.Location),
                FormatPoint(target.Location),
                currentDistance,
                previousDistanceToTarget,
                distanceDelta,
                SafeLog(movementPrimitive),
                creature != null ? creature.ControlOrder.ToString() : "none",
                creature != null && creature.ControlTarget != null ? DescribeMobile(creature.ControlTarget as Mobile) : "none",
                creature != null && creature.Combatant != null ? DescribeMobile(creature.Combatant as Mobile) : "none",
                SafeLog(state.ActionMode.ToString()),
                SafeLog(state.LastMoveDirection),
                companion.Location != previousPoint);
        }
        ...
        private static string BuildActionStartLine(BaseHire companion, AIGMCompanionTrackingState state)
        {
            ...
            if (state != null && state.Mode == AIGMCompanionTrackingMode.Monsters && state.ActionMode == AIGMCompanionTrackingActionMode.TrackMove)
                return String.Format("{0} begins tracking nearby monsters.", SafeSpeech(companion != null ? companion.Name : null, "The companion"));
        ...
        {
            return String.Format("{0} stopped tracking.", SafeSpeech(companion != null ? companion.Name : null, "The companion"));
        }
        ...
        private static string BuildInactiveStatusLine(BaseHire companion, AIGMCompanionTrackingState state)
        {
            ...
            return String.Format("{0} is not tracking anything. {1}.", SafeSpeech(companion != null ? companion.Name : null, "The companion"), capability);
        }
        ...
        private static string BuildNoTrailLine(BaseHire companion, AIGMCompanionTrackingMode mode)
        {
            ...
            if (mode == AIGMCompanionTrackingMode.Monsters)
                return String.Format("{0} found no monster trail nearby.", SafeSpeech(companion != null ? companion.Name : null, "The companion"));
        ...
        private static string BuildTrailLine(BaseHire companion, string category, string name, string direction, string distance, string tile, int count)
        {
            ...
            if (String.Equals(category, "monster", StringComparison.OrdinalIgnoreCase))
                return String.Format("{0} is tracking {1}{2}.", SafeSpeech(companion != null ? companion.Name : null, "The companion"), SafeSpeech(name, "a monster"), where);
