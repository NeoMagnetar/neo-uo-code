...
namespace Server.Custom.AIGM
{
    public static class AIGMCompanionTurnCoordinator
    {
        ...
        public static AIGMCompanionPartySpeechContext BuildContext(IAIGMCompanionActor companion, Mobile speaker, string rawSpeech, string dialogueMode, string originCompanionId, int hopCount, string dialogueTargetCompanionId)
        {
            ...
            context.AddressingMode = groupAddressed
                ? "ExplicitGroup"
                : (!String.IsNullOrWhiteSpace(context.AddressedCompanionId) ? "ExactActor" : "Unaddressed");
            context.PrimaryCanonicalId = context.AddressedCompanionId ?? String.Empty;
            context.AddressedGroupId = groupAddressed ? "companions" : String.Empty;
            context.AllowSecondaryResponse = false;
            context.SecondaryResponseLimit = 0;
        ...
        public static bool ShouldSuppressOwnerSpeechRelay(string rawSpeech)
        {
            if (IsGroupAddressed(rawSpeech) || IsOwnerDirectedCompanionDialogue(rawSpeech))
                return false;

            List<string> addressed = AIGMCompanionCommandBoundary.GetAddressedCompanionIds(rawSpeech);
            return addressed != null && addressed.Count == 1;
        }
        ...
        private static void SelectResponders(AIGMCompanionPartySpeechContext context, List<IAIGMCompanionActor> listeners, AIGMCompanionCommandRouteDecision route, string originCompanionId, Mobile speaker)
        {
                ...
                context.DialogueMode == AIGMCompanionDialogueMode.SystemStatus)
            {
                ...
                context.TurnCoordinatorDecision = context.GroupAddressed || IsPlayerPartyBroadcast(context, speaker)
                    ? "group_bounded_primary_only"
        ...
        private static bool ShouldFanOutToWholeParty(AIGMCompanionPartySpeechContext context, Mobile speaker)
        {
            if (context == null)
                return false;

            if (context.GroupAddressed)
                return true;

            return IsPlayerPartyBroadcast(context, speaker);
        }
        ...
        private static void SelectCompanionDialogueResponder(AIGMCompanionPartySpeechContext context, List<IAIGMCompanionActor> listeners, string originCompanionId, Mobile speaker)
        {
            ...
            if (!String.IsNullOrWhiteSpace(preferredTargetId))
            {
                ...
                context.TurnCoordinatorDecision = selectedCount > 1 ? "companion_dialogue_targeted_multi_followup" : (selected ? "companion_dialogue_targeted_followup" : "PHASE58D-ECHO-BLOCKED");
            ...
            {
                IAIGMCompanionActor actor = listeners[i];
                if (actor == null || context.IsSelected(actor.CompanionId) || String.Equals(actor.CompanionId, speakerId, StringComparison.OrdinalIgnoreCase))
                    continue;

                context.Suppress(actor.CompanionId, "companion_dialogue_bounded_secondary_limit");
            }

            context.TurnCoordinatorDecision = broadcastSelections > 1 ? "companion_dialogue_party_followup" : (selected ? "companion_dialogue_one_followup" : "PHASE58D-ECHO-BLOCKED");
            if (!selected && String.IsNullOrWhiteSpace(context.EchoSuppressionReason))
                context.EchoSuppressionReason = "no_followup_selected";
        }

        private static void SetPrimary(AIGMCompanionPartySpeechContext context, IAIGMCompanionActor actor)
        {
            if (context == null || actor == null || actor.Shell == null)
                return;

            if (context.PrimaryActorSerial != Serial.MinusOne)
                return;

            context.PrimaryActorSerial = actor.Shell.Serial;
            context.PrimaryCanonicalId = actor.CompanionId ?? context.PrimaryCanonicalId ?? String.Empty;
        }
