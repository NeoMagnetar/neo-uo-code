...
namespace Server.Custom.AIGM
{
    public static class AIGMStubResponder
    {
        public static AIGMResponse Ask(Mobile from, string question, AIGMTargetInfo target)
        {
            ...
            TryAddPropsInspectionActions(response, normalized, lowered);
        ...
        private static void TryAddMovementActions(AIGMResponse response, Mobile from, string normalized, string lowered, AIGMTargetInfo target)
        {
            ...
            if (ContainsAny(lowered, "follow me", "come here", "go to me", "stop following", "stop moving", "resume following", "resume movement"))
                return;
        ...
        private static void TryAddPropsInspectionActions(AIGMResponse response, string normalized, string lowered)
        {
            if (response == null || String.IsNullOrWhiteSpace(lowered))
                return;

            if (!(ContainsAny(lowered, "inspect", "examine", "read properties", "read props", "what is")))
                return;

            string targetMode = "nearest_mobile";
            if (ContainsAny(lowered, "nearest container", "container"))
                targetMode = "nearest_container";
            else if (ContainsAny(lowered, "nearest door", "door"))
                targetMode = "nearest_door";
            else if (ContainsAny(lowered, "nearest item", "item", "sword", "katana", "spellbook", "gold"))
                targetMode = "nearest_item";

            AIGMActionProposal inspect = new AIGMActionProposal();
            inspect.ActionKind = "gm_props_read";
            inspect.Title = "Inspect deeply";
            inspect.Description = "Read full native properties for the identified target.";
            inspect.Category = "read";
            inspect.PreviewText = "Opens a richer exact inspection view with type, serial, stats, and object details.";
            inspect.RequiresConfirmation = false;
            inspect.EnsureParameters();
            inspect.Parameters["commandSurface"] = "Props";
            inspect.Parameters["targetMode"] = targetMode;

            response.ProposedActions.Add(inspect);
            response.NextAction = response.NextAction ?? inspect;
        }
