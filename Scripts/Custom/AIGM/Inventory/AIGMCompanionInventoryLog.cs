...
{
    public static class AIGMCompanionInventoryLog
    {
        ...
        public static void Write(string eventName, Mobile actor, IDictionary<string, string> fields)
        {
            try
            {
                string dir = Path.Combine(Core.BaseDirectory, "Logs", "AIGM", "Inventory");
                ...
                AppendJson(sb, "actor", actor != null ? Safe(actor.Name ?? actor.GetType().Name) : String.Empty, false);
                AppendJson(sb, "actorSerial", actor != null ? FormatSerial(actor.Serial) : String.Empty, false);
                ...
                lock (SyncRoot)
                {
                    File.AppendAllText(Path.Combine(dir, "companion_inventory.jsonl"), sb.ToString() + Environment.NewLine, Encoding.UTF8);
                }
            }
            catch
            {
        ...
        public static string FormatSerial(Serial serial)
        {
