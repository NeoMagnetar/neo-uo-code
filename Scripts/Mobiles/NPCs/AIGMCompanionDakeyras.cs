...
namespace Server.Mobiles
{
    public class AIGMCompanionDakeyras : BaseHire
    {
            ...
            if (!e.Handled && e.Mobile.InRange(this, 8))
            {
                ...
                if (trusted)
                {
                    ...
                    if (parsed)
                    {
                        ...
                        {
                            LogSpeech("DAK_DIRECT_EXECUTE_RESULT kind=" + (intent != null ? intent.Kind : "null") + " executed=False response=Recognized support intent blocked from async fallback.");
                            SayTo(e.Mobile, "I cannot do that just now.");
                            e.Handled = true;
                        }
                    }
        ...
        public void ReceiveSpeechBusEvent(BaseHire sourceCompanion, Mobile eventSpeaker, string speech, bool companionOrigin)
        {
            ...
            if (companionOrigin && speech.IndexOf(Name ?? "Dakeyras", StringComparison.OrdinalIgnoreCase) < 0)
                handledSpeech = (Name ?? "Dakeyras") + ", " + speech;
            ...
            LogSpeech("DAK_SPEECH_BUS_QUEUE enqueued=" + enqueued + " rejection=" + (rejection ?? String.Empty));
        }
