...
namespace Server.Custom.AIGM
{
    public static class AIGMCompanionMapNavigator
    {
        ...
        public static bool TryChooseWallFollowPoint(BaseHire companion, Point3D goal, AIGMCompanionTravelObjective objective, out Point3D wallPoint)
        {
            ...
            int lateralX = goalDx != 0 ? -goalDx : 0;
            int lateralY = goalDy != 0 ? -goalDy : 0;
            ...
            {
                candidates = new Point3D[]
                {
                    MakePoint(companion.Map, companion.X, companion.Y + goalDy, companion.Z),
                    MakePoint(companion.Map, companion.X + lateralX, companion.Y + goalDy, companion.Z),
                    MakePoint(companion.Map, companion.X + lateralX, companion.Y, companion.Z),
                    MakePoint(companion.Map, companion.X + lateralX, companion.Y + goalDy + goalDy, companion.Z),
                    MakePoint(companion.Map, companion.X, companion.Y + goalDy + goalDy, companion.Z)
                };
                chosenDirX = lateralX;
                chosenDirY = goalDy;
            }
            else if (goalDx != 0)
            {
                candidates = new Point3D[]
                {
                    MakePoint(companion.Map, companion.X, companion.Y + 1, companion.Z),
                    MakePoint(companion.Map, companion.X, companion.Y - 1, companion.Z),
                    MakePoint(companion.Map, companion.X + lateralX, companion.Y + 1, companion.Z),
                    MakePoint(companion.Map, companion.X + lateralX, companion.Y - 1, companion.Z),
                    MakePoint(companion.Map, companion.X + lateralX, companion.Y, companion.Z)
                };
                chosenDirX = lateralX;
            ...
            else
            {
                candidates = new Point3D[]
                {
                    MakePoint(companion.Map, companion.X + 1, companion.Y, companion.Z),
                    MakePoint(companion.Map, companion.X - 1, companion.Y, companion.Z),
                    MakePoint(companion.Map, companion.X + 1, companion.Y + lateralY, companion.Z),
                    MakePoint(companion.Map, companion.X - 1, companion.Y + lateralY, companion.Z),
                    MakePoint(companion.Map, companion.X, companion.Y + lateralY, companion.Z)
                };
                chosenDirX = 0;
                chosenDirY = lateralY;
            }
            ...
            for (int i = 0; i < candidates.Length; i++)
            {
                ...
                if (CanStandAt(companion.Map, candidate.X, candidate.Y, candidate.Z))
                {
                    objective.WallFollowDirectionX = chosenDirX;
                    objective.WallFollowDirectionY = chosenDirY;
                    wallPoint = candidate;
                    return true;
                }
            }
