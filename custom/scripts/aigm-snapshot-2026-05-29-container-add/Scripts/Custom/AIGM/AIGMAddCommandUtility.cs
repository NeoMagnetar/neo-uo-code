...
namespace Server.Custom.AIGM
{
    ...
    public static class AIGMAddCommandUtility
    {
        ...
        public static AIGMAddCreateResult TryCreateAndPlaceInContainer(Mobile requester, AIGMConstructableResolution resolution, int amount, Container targetContainer)
        {
            if (requester == null || requester.Deleted)
                return AIGMAddCreateResult.Fail("Requester is missing or deleted.");

            if (targetContainer == null || targetContainer.Deleted)
                return AIGMAddCreateResult.Fail("Target container is missing or deleted.");
            ...
            for (int i = 0; i < ctors.Length; ++i)
            {
                ...
                if (!targetContainer.TryDropItem(requester, item, false))
                {
                    item.Delete();
                    return AIGMAddCreateResult.Fail("The target container could not receive that item.");
                }

                return AIGMAddCreateResult.Success(
                    String.Format("Created {0} in container {1}.", item.GetType().Name, targetContainer.GetType().Name),
                    item.Serial,
                    item.GetType().Name);
            }

            return AIGMAddCreateResult.Fail("No supported native Add path succeeded for container item type: " + resolution.CanonicalTypeName);
        }
