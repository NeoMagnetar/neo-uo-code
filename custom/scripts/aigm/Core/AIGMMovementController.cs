...
namespace Server.Custom.AIGM
{
    ...
    public static class AIGMMovementController
    {
        ...
        {
            message = null;
            if (from == null || counselor == null || map == null)
            {
                message = "Pathing could not start because the counselor or map was missing.";
                return false;
            }

            if (counselor.Map != map)
            {
            ...
            counselor.CantWalk = false;
            counselor.ControlTarget = null;
            counselor.ControlOrder = OrderType.Stop;
            counselor.Home = adjusted;
            counselor.RangeHome = 0;
            ...
            message = String.Format("{0} is moving to {1}.", counselor.Name ?? "The counselor", !String.IsNullOrWhiteSpace(destinationName) ? destinationName : String.Format("{0},{1},{2}", adjusted.X, adjusted.Y, adjusted.Z));
            return true;
        }
        ...
        private static void TickPath(Mobile owner, BaseCreature counselor, AIGMMovementState state)
        {
            ...
            bool moved = StepToward(counselor, destination, shouldRun);
            ...
            {
                string ignore;
                Stop(owner, counselor, String.Format("Blocked before reaching {0}", !String.IsNullOrWhiteSpace(state.DestinationName) ? state.DestinationName : FormatPoint(destination)), out ignore);
                return;
            }

            state.LastStatus = "Pathing around obstacle";
        }
