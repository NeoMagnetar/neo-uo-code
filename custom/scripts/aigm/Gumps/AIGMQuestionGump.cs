...
{
if (m_State.LastResponse.Warnings == null)
m_State.LastResponse.Warnings = new List<string>();

m_State.LastResponse.Warnings.Add("DEBUG: Stub replacement bypassed so the live bridge error stays visible.");
Log("Stub replacement bypassed for debugging.");
}

                            Log("Sending response gump.");
                            m_From.SendGump(new AIGMResponseGump(m_From, m_Npc, m_State.LastResponse));
                            Log("Response gump sent.");
                            break;
                        }
                        catch (Exception ex)
                        {
