...
$serverRoot = 'C:\UO\Server\Neo Ultima Online\NeoUO-Staging'
$serverExe = Join-Path $serverRoot 'ServUO.exe'
$serverPort = 2595
$serverTitle = 'NeoUO Staging Server'
...
$envSettings = Join-Path $clientRoot 'settings.staging.json'
...
if (-not (Test-PortListening $serverPort)) { throw 'NeoUO Staging server did not start on port 2595.' }
